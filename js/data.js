...
window.Data = (function () {
  ...
  const ENEMIES = {
    ...
    shark:  { name: 'Maneater Shark',   model: 'shark',  hp: 88,  xp: 24, gold: 18, baseY: 0,   moves: [ { name: 'sinks its teeth in', min: 11, max: 17 }, { name: 'whips its tail', min: 14, max: 21 }, { name: 'thrashes into a feeding frenzy', min: 18, max: 27 } ], drops: [ { mat: 'fang', chance: 0.7 }, { mat: 'fin', chance: 0.4 } ] },
    crab:   { name: 'Giant Hermit Crab', model: 'crab',  hp: 118, xp: 28, gold: 22, baseY: 0,   moves: [ { name: 'snaps a giant claw', min: 13, max: 19 }, { name: 'bashes with its shell', min: 16, max: 23 }, { name: 'foams a stinging spray', min: 9, max: 14, all: true, el: 'water' } ], drops: [ { mat: 'shellfrag', chance: 0.75 }, { mat: 'brine', chance: 0.2 } ] },
    jelly:  { name: "Man-o'-War Jelly",  model: 'jelly', hp: 60,  xp: 20, gold: 13, baseY: 0.2, moves: [ { name: 'stings sharply', min: 9, max: 15 }, { name: 'discharges a shock', min: 9, max: 14, all: true, el: 'thunder' }, { name: 'wraps you in numbing tendrils', min: 10, max: 15, status: 'slow', turns: 2 } ], drops: [ { mat: 'goo', chance: 0.85 } ] },
    octo:   { name: 'Reef Octopus',      model: 'octo',  hp: 102, xp: 30, gold: 24, baseY: 0.1, moves: [ { name: 'slams a tentacle', min: 15, max: 21 }, { name: 'sprays stinging ink', min: 8, max: 12, all: true }, { name: 'constricts a victim', min: 16, max: 23, status: 'slow', turns: 2 } ], drops: [ { mat: 'ink', chance: 0.7 }, { mat: 'goo', chance: 0.3 } ] },
    gull:   { name: 'Dive-Bomb Gull',    model: 'gull',  hp: 54,  xp: 18, gold: 12, baseY: 1.4, moves: [ { name: 'pecks rapidly', min: 8, max: 13 }, { name: 'dive-bombs', min: 17, max: 25 }, { name: 'kicks sand in your eyes', min: 8, max: 12, all: true, status: 'weaken', turns: 2 } ], drops: [ { mat: 'feather', chance: 0.85 } ] },
    golem:  { name: 'Sandcastle Golem',  model: 'golem', hp: 136, xp: 36, gold: 30, baseY: 0,   moves: [ { name: 'slams a sandy fist', min: 15, max: 23 }, { name: 'crumbles down', min: 11, max: 18, all: true }, { name: 'hurls a heavy boulder', min: 17, max: 25, el: 'earth' } ], drops: [ { mat: 'sand', chance: 0.85 }, { mat: 'brine', chance: 0.25 } ] },
    eel:    { name: 'Voltaic Eel',       model: 'eel',   hp: 96,  xp: 32, gold: 24, baseY: 1.0, moves: [ { name: 'lashes its tail', min: 13, max: 19 }, { name: 'looses a numbing current', min: 12, max: 18, all: true, status: 'slow', turns: 3 }, { name: 'discharges a voltaic burst', min: 16, max: 23, el: 'thunder' } ], drops: [ { mat: 'goo', chance: 0.6 }, { mat: 'brine', chance: 0.35 } ] },
    cobra:  { name: 'Sand Cobra',        model: 'cobra', hp: 92,  xp: 30, gold: 22, baseY: 0,   moves: [ { name: 'strikes with dripping fangs', min: 13, max: 20, status: 'poison', turns: 3 }, { name: 'spits venom across', min: 11, max: 16, all: true, status: 'poison', turns: 3 }, { name: 'coils and constricts', min: 16, max: 23 } ], drops: [ { mat: 'fang', chance: 0.65 }, { mat: 'sand', chance: 0.4 } ] },
    scarab: { name: 'Gilded Scarab',     model: 'scarab', hp: 120, xp: 34, gold: 34, baseY: 0.2, moves: [ { name: 'slashes with golden pincers', min: 14, max: 21 }, { name: 'scatters a blinding glare', min: 10, max: 15, all: true, status: 'weaken', turns: 3 }, { name: 'burrows and erupts beneath you', min: 16, max: 23 } ], drops: [ { mat: 'shellfrag', chance: 0.6 }, { mat: 'brine', chance: 0.4 } ] },
    ...
    wyvern: { name: 'Vale Wyvern',      model: 'wyvern', hp: 128, xp: 38, gold: 30, baseY: 1.2, moves: [ { name: 'rakes with talons', min: 15, max: 22 }, { name: 'breathes a gout of flame', min: 13, max: 19, all: true, el: 'fire' }, { name: 'buffets with its wings', min: 15, max: 22 } ], drops: [ { mat: 'abyssscale', chance: 0.4 }, { mat: 'fang', chance: 0.5 } ] },
    ...
    urchin: { name: 'Spine Urchin',      model: 'urchin', hp: 150, xp: 34, gold: 26, baseY: 0,  moves: [ { name: 'fires a spine volley', min: 14, max: 20 }, { name: 'rolls over the party', min: 12, max: 18, all: true }, { name: 'embeds a venom spine', min: 13, max: 19, status: 'poison', turns: 2 } ], drops: [ { mat: 'shellfrag', chance: 0.6 }, { mat: 'sand', chance: 0.4 } ] },
    // ---- Paegina (Greek myth) bestiary ----
    harpy:  { name: 'Shrieking Harpy',   model: 'harpy', hp: 96,  xp: 34, gold: 26, baseY: 1.5, moves: [ { name: 'rakes with talons', min: 14, max: 20 }, { name: 'looses a deafening shriek', min: 11, max: 16, all: true }, { name: 'snatches a target aloft and drops it', min: 18, max: 25 } ], drops: [ { mat: 'feather', chance: 0.8 }, { mat: 'fang', chance: 0.3 } ] },
    satyr:  { name: 'Wine-Mad Satyr',    model: 'satyr', hp: 110, xp: 36, gold: 30, baseY: 0,   moves: [ { name: 'gores with its horns', min: 15, max: 22 }, { name: 'plays a maddening reel', min: 10, max: 15, all: true, status: 'slow', turns: 3 }, { name: 'guzzles wine and revels', heal: true } ], drops: [ { mat: 'fang', chance: 0.5 }, { mat: 'goo', chance: 0.4 } ] },
    ...
    thingspawn: { name: 'Thing-Spawn',   model: 'thingspawn', hp: 124, xp: 40, gold: 28, baseY: 0, moves: [ { name: 'lashes a fused limb', min: 15, max: 22 }, { name: 'tries to assimilate flesh', min: 12, max: 18, status: 'poison', turns: 3 }, { name: 'splits apart and reforms', heal: true } ], drops: [ { mat: 'goo', chance: 0.8 }, { mat: 'ectoplasm', chance: 0.4 } ] },
    kodama:  { name: 'Kodama',           model: 'kodama', hp: 64,  xp: 26, gold: 18, baseY: 0, moves: [ { name: 'rattles its hollow head', min: 9, max: 14, status: 'slow', turns: 2 }, { name: 'calls the wood to bind', min: 11, max: 16, all: true }, { name: 'curses with forest spite', min: 12, max: 18, el: 'dark' } ], drops: [ { mat: 'feather', chance: 0.5 }, { mat: 'goo', chance: 0.3 } ] },
    ...
    sentry:  { name: 'Sentry Drone',     model: 'sentry', hp: 88,  xp: 34, gold: 26, baseY: 1.4, moves: [ { name: 'fires a scanning beam', min: 14, max: 20 }, { name: 'paints all targets', min: 10, max: 15, all: true }, { name: 'overloads a capacitor', min: 16, max: 23, el: 'thunder' } ], drops: [ { mat: 'brine', chance: 0.5 }, { mat: 'shellfrag', chance: 0.3 } ] },
    mutton:  { name: 'Battle-Mutton',    model: 'mutton', hp: 96,  xp: 24, gold: 16, baseY: 0, moves: [ { name: 'headbutts indignantly', min: 11, max: 17 }, { name: 'stampedes the flock', min: 13, max: 19, all: true }, { name: 'lets out a withering bleat', min: 9, max: 13, all: true, status: 'weaken', turns: 2 } ], drops: [ { mat: 'feather', chance: 0.4 }, { mat: 'sand', chance: 0.3 } ] },
    ...
    bat:    { name: 'Nightwing Bat',     model: 'bat',   hp: 78,  xp: 30, gold: 20, baseY: 1.6, moves: [ { name: 'bites with a screech', min: 12, max: 18 }, { name: 'drains warm blood', min: 14, max: 20 }, { name: 'swarms in a screeching cloud', min: 10, max: 15, all: true, el: 'dark' } ], drops: [ { mat: 'ectoplasm', chance: 0.4 }, { mat: 'feather', chance: 0.3 } ] },
    ghoul:  { name: 'Drowned Ghoul',     model: 'ghoul', hp: 132, xp: 38, gold: 28, baseY: 0,   moves: [ { name: 'rakes with rotted claws', min: 15, max: 22 }, { name: 'exhales grave-rot', min: 12, max: 18, all: true }, { name: 'infects an open wound', min: 14, max: 20, status: 'poison', turns: 3 } ], drops: [ { mat: 'ectoplasm', chance: 0.7 }, { mat: 'brine', chance: 0.3 } ] },
    wraith: { name: 'Tide Wraith',       model: 'wraith', hp: 110, xp: 40, gold: 30, baseY: 0.6, moves: [ { name: 'phases through a soul', min: 16, max: 23 }, { name: 'wails a dirge', min: 13, max: 19, all: true }, { name: 'chills the soul', min: 16, max: 22, el: 'dark', status: 'slow', turns: 2 } ], drops: [ { mat: 'ectoplasm', chance: 0.8 } ] },
  ...
  const ITEM_DEFS = {
    ...
    quakestone:{ name: 'Quake Stone', kind: 'damage', min: 65, max: 92, target: 'enemy', fx: 'fire', el: 'earth', price: 150, desc: '65-92 Earth damage to one foe.' },
    voidshard:{ name: 'Void Shard',   kind: 'damage', min: 72, max: 98, target: 'enemy', fx: 'beam', el: 'dark', price: 200, desc: '72-98 Dark damage to one foe.' },
    mendsalve:{ name: 'Mending Salve', kind: 'buff',  status: 'regen', turns: 4, target: 'ally', fx: 'heal', price: 160, desc: 'Grants Regen to one ally for a few turns.' },
    frostmine:{ name: 'Frost Mine',   kind: 'damageall', min: 42, max: 62, target: 'enemyall', fx: 'water', el: 'water', price: 260, desc: '42-62 Water damage to ALL foes.' },
    stormmine:{ name: 'Storm Mine',   kind: 'damageall', min: 48, max: 70, target: 'enemyall', fx: 'beam', el: 'thunder', price: 300, desc: '48-70 Thunder damage to ALL foes.' },
  };
  ...
  const SHOP_STOCK_BY_TOWN = {
    ...
    mall:      ['potion', 'hipotion', 'xpotion', 'ether', 'hiether', 'turboether', 'phoenix', 'megaphoenix', 'elixir', 'adrenaline', 'megapotion', 'grenade', 'frostmine', 'stormmine', 'holybomb', 'boltbomb', 'frostbomb', 'quakestone', 'voidshard', 'antidote', 'remedy', 'herodrink', 'hourglass', 'mendsalve'],
    bazaar:    ['potion', 'hipotion', 'xpotion', 'ether', 'hiether', 'phoenix', 'bomb', 'frostbomb', 'boltbomb', 'quakestone', 'antidote', 'remedy', 'herodrink', 'hourglass', 'mendsalve'],
  ...
  const RECIPES = [
    ...
    { out: 'quakestone', qty: 1, cost: { sand: 3, shellfrag: 1 },     name: 'Quake Stone' },
    { out: 'voidshard',  qty: 1, cost: { ectoplasm: 2, ink: 1 },      name: 'Void Shard' },
    { out: 'mendsalve',  qty: 2, cost: { goo: 2, feather: 1 },        name: 'Mending Salve ×2' },
    { out: 'frostmine',  qty: 1, cost: { brine: 2, fin: 1, sand: 1 }, name: 'Frost Mine' },
    { out: 'stormmine',  qty: 1, cost: { goo: 3, ink: 1, brine: 1 },  name: 'Storm Mine' },
  ];
  ...
  const STORY = {
    ...
    opening: [
      { name: 'Narrator', text: 'For a thousand years the coast of Saltmere knew only gentle tides and golden mornings. The Free Seas were a promise: that anyone, from any shore, could chase a horizon and call it home.', shot: 'wide', cut: true },
      { name: 'Narrator', text: 'Then the sea began to RISE. Not in a day — in a slow, drowning patience. Beaches vanished. Beasts crawled from the foam. And the people learned a name to be afraid of.', shot: 'high', cut: true },
      { name: 'Narrator', text: 'SELACHOTH. The One-Finned Angel. Once the greatest hero the Free Seas ever produced — now half-man, half-shark, and wholly convinced that a warm world of the living is a mistake the ocean must correct.', shot: 'wide' },
      { name: 'Lance Strider', text: 'He was my mentor. I watched the sea take him piece by piece, and I told myself a hero couldn\'t fall. I was wrong. So I\'ll be the one to put him down.', shot: 'low', cut: true, act: { do: 'point' } },
      { name: 'Marina', text: 'And I am sworn to the tides themselves. They weep under his hand. I\'ll not let the deep be turned into a tomb.', shot: 'closeup', cut: true },
      { name: 'Capt. Redbeard', text: 'Bah — speeches. I just want my coast back, and my grog dry. And would you LOOK at that — there she is, fitted and floating, waiting on the tide.', shot: 'over', cut: true, act: { do: 'point' } },
      { name: 'Marina', text: 'The sun\'s breaking over the water. If we\'re ever going to do this... it\'s now.', shot: 'hero' },
      { name: 'Lance Strider', text: 'Then no more waiting. Every drowned isle east of here is a clock, and it\'s already ticking.', shot: 'closeup', cut: true },
      { name: 'Capt. Redbeard', text: 'HAH! Look at us — three fools on a cliff with one ship and a death wish. I\'ve started worse crews with worse odds. WE\'RE READY. Hoist the colours — we\'re going on an ADVENTURE!', shot: 'low', cut: true, move: { to: [0, -2.2], ms: 700 }, act: { do: 'cheer' } },
      { name: 'Narrator', text: 'The road east is long, and you will not walk it alone. Some who join you chase glory, some chase ghosts — and one chases a dream so bright it will cost him everything. But that is all still ahead. For now: a cliff, a dawn, a ship. Cast off.', shot: 'wide' },
    ],
    ruffyChase: [
